...
void deriveSF()
{
  ...
  // Inputs
  string common_selection = "(Diphoton_sublead_pt_mgg>0.35)*(Diphoton_lead_pt_mgg>0.25)*(CMS_hgg_mass<115 || CMS_hgg_mass>135)*(minID>-0.7)*(category==1)";
  ...
  for( auto filenameitr : filenames_map) {
    ...
    for( unsigned ifile=0; ifile<filenames.size(); ++ifile) {
      ...
      ch->Draw( Form("minID >>+ h_minphotonID_%s",samplename.c_str()),
		Form("(weight_central)*(%f)*(%s)",lumis[ifile],common_selection.c_str()),
		"goff");
      ch->Draw( Form("maxID >>+ h_maxphotonID_%s",samplename.c_str()),
		Form("(weight_central)*(%f)*(%s)",lumis[ifile],common_selection.c_str()),
		"goff");
