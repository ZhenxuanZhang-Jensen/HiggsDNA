...
class HHWW_Preselection(Tagger):
    ...
    def calculate_selection(self, events):
        ...
        # ---------------------------------------------------------------------------- #
        #               add lepton in output parquet             #
        # ---------------------------------------------------------------------------- #   
        # select_e = events[events.electron_pt!=-999]
        # select_mu = events[events.muon_pt!=-999]
        # lepton_p4 = vector.awk(
        #     {
        #         "pt" : awkward.pad_none(awkward.concatenate([select_e.electron_pt,select_mu.muon_pt],axis=0),len(events),axis=0),
        #         "eta" : awkward.pad_none(awkward.concatenate([select_e.electron_eta,select_mu.muon_eta],axis=0),len(events),axis=0),
        #         "phi" : awkward.pad_none(awkward.concatenate([select_e.electron_phi,select_mu.muon_phi],axis=0),len(events),axis=0),
        #         "mass" : awkward.pad_none(awkward.concatenate([select_e.electron_mass,select_mu.muon_mass],axis=0),len(events),axis=0)
        #     },
        #     with_name = "Momentum4D")
        # lepton = awkward_utils.add_field(
        #     events=events,
        #     name="lepton",
        #     data=lepton_p4,
        # )  
