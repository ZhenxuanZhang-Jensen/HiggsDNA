...
class HHWW_Preselection_FHSL(Tagger):
    ...
    def calculate_selection(self, events):
        ...
        fatjetTop = fatjet_selections.select_fatjets(
            fatjets = events.FatJet,
            subjets = events.SubJet,
            options = self.options["fatjetTop"],
            clean = {
                "photons" : {
                    "objects" : events.Diphoton.Photon,
                    "min_dr" : self.options["fatjetTop"]["dr_photons"]
                },
                "electrons" : {
                    "objects" : events.SelectedElectron_iso,
                    "min_dr" : self.options["fatjetTop"]["dr_electrons"]
                },
                "muons" : {
                    "objects" : events.SelectedMuon_iso,
                    "min_dr" : self.options["fatjetTop"]["dr_muons"]
                    },
                },
            name = "SelectedFatJetTop",
            tagger = self
        )

        

        # add lead and sublead subjet btag scores in fatjetTop
        subjets = events.SubJet

        fatjetTop = awkward_utils.add_field(
            events = events,
            name = "SelectedFatJetTop",
            data = events.FatJet[fatjetTop]
        )   
        fatjetTop = fatjetTop[awkward.argsort(fatjetTop.Hqqqq_vsQCDTop, ascending=False, axis=-1)]
        fatjetTop = awkward.pad_none(fatjetTop, 1, clip=True)

        print("fatjetTop fields", fatjetTop.fields)
        print("fatjetTop", fatjetTop)
        print("type fatjetTop", type(fatjetTop))
        print("((awkward.fill_none(awkward.pad_none(subjets.btagDeepB,1,axis=1),-999,axis=-1)[awkward.fill_none(awkward.pad_none(fatjetTop.subJetIdx1,1,axis=1),0,axis=-1)]))", ((awkward.fill_none(awkward.pad_none(subjets.btagDeepB,1,axis=1),-999,axis=-1)[awkward.fill_none(awkward.pad_none(fatjetTop.subJetIdx1,1,axis=1),0,axis=-1)])))
        events['fatjetTop_leadSubjet_btagDeepB'] = ((awkward.fill_none(awkward.pad_none(subjets.btagDeepB,1,axis=1),-999,axis=-1)[awkward.unflatten(awkward.flatten(awkward.fill_none(awkward.pad_none(fatjetTop.subJetIdx1,1,axis=1),0,axis=-1)),awkward.num(fatjetTop))]))
        events['fatjetTop_subleadSubjet_btagDeepB'] = ((awkward.fill_none(awkward.pad_none(subjets.btagDeepB,1,axis=1),-999,axis=-1)[awkward.unflatten(awkward.flatten(awkward.fill_none(awkward.pad_none(fatjetTop.subJetIdx2,1,axis=1),0,axis=-1)),awkward.num(fatjetTop))]))

        print("after fatjetTop fields", fatjetTop.fields)
        print("after fatjetTop", fatjetTop)
        print("after type fatjetTop", type(fatjetTop))

        awkward_utils.add_object_fields(
        events=events,
        name="fatjetTop",
        # objects=fatjetTop[awkward.argsort(fatjetTop.Hqqqq_qqlv_vsQCDTop, ascending=False, axis=-1)],
        objects=fatjetTop,
        n_objects=1,
        dummy_value=-999
        ) 
        ...
        n_fatjetsTop = awkward.sum(awkward.fill_none(fatjetTop.pt, -999) > 0, axis=-1)
        awkward_utils.add_field(events,"nGoodAK8jets",n_fatjets)
        # awkward_utils.add_field(events,"nGood_W_fatjets",n_fatjets_W)
        awkward_utils.add_field(events,"nGood_Top_fatjets",n_fatjetsTop)
        ...
        SL_merged_boosted_cat = (n_leptons_noiso == 1) & (n_leptons_iso==0) & (selection_lepton_merged_SL_cat0) &(awkward.num((dR_lep_fatjet==True)>=1))# & (awkward.num((dphi_MET_fatjet==True)>=1)) # merged boosted 1 jet for SL channel wo isolated lep
        # ----------------------------------------------------------------------------------------------------#
        # If no lepton
        ...
        # selection_fatjet_HvsQCD_FH_cat0 = awkward.num(fatjetTop.Hqqqq_vsQCDTop[(fatjetTop.Hqqqq_vsQCDTop > 0.4)]) >= 1
        FH_boosted = (n_leptons_iso == 0) & (n_leptons_noiso == 0) & (n_fatjetsTop >=1)# &(awkward.num((dphi_MET_fatjet==True)==0)) # boosted 1 jet for SL and FH channel wo isolated lep
        # ----------------------------------------------------------------------------------------------------#
        ...
        FH_fully_resovled_cat =(n_leptons_iso==0) & (n_leptons_noiso == 0) & (n_jets>=2)# 4 jets for FH
        ...
        category_cut = (category == 4) # cut the events with category == 0
