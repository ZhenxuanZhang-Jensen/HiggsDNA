...
class HHWW_Preselection_FHSL(Tagger):
    ...
    def calculate_selection(self, events):
        ...
        FH_cat0 = (n_leptons == 0) & (n_fatjets_H >=1) & (selection_fatjet_HvsQCD_FH_cat0) # boosted 1 jet for SL and FH channel wo isolated lep
        ...
        FH_SB_2Fatjet = (n_leptons==0) & (n_fatjets >=2) & (selection_fatjet_WvsQCD_SB_2F) # 2 jets for FH
        ...
        FH_SB_1Fatjet = (n_leptons==0) & (n_fatjets ==1) & (n_jets >=2) & (selection_fatjet_WvsQCD_SB_1F) # 1 jet for FH

        # add resolved FH category with (>=4 AK4 jets )
        FH_fully_resovled_cat = (n_leptons==0) & (n_jets>=4) # 4 jets for FH
        ...
        category = awkward.where(FH_SB_1Fatjet, awkward.ones_like(category)*3, category)
        category = awkward.where(FH_SB_2Fatjet, awkward.ones_like(category)*2, category)
        category = awkward.where(FH_cat0, awkward.ones_like(category)*1, category)
