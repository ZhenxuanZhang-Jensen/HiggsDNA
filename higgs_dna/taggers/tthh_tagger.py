...
class TTHHTagger(Tagger):
    ...
    def __init__(self, name, options = {}, sample = None):
        super(TTHHTagger, self).__init__(name, options, sample)
    ...
    def calculate_selection(self, syst_tag, syst_events):
        ...
        presel_cut = hadronic | semi_lep | semi_tau | dilepton | multilep

        self.register_cuts(
            names = ["hadronic", "semi_lep", "semi_tau", "dilepton", "multilepton", "inclusive"],
            results = [hadronic, semi_lep, semi_tau, dilepton, multilep, presel_cut]
        )
