...
class HHWW_Preselection_FHSL(Tagger):
    ...
    def calculate_selection(self, events):
        ...
        muon_fatjet_deltaR = delta_R(muons, fatjets, 0)
        muon_fatjet_deltaR_selection = awkward.num(muon_fatjet_deltaR) >= 1 # at least one muon deltaR with all fatjets < 0.8

        SL_WvsQCD_tight_muon_cat = (n_leptons == 1) & (n_muons == 1) & (muon_fatjet_deltaR_selection) & (selection_fatjet_WvsQCD_tight_muon) # boosted 1 jet for SL and FH channel wo isolated lep
        ...
        category = awkward.where(SL_cat0, awkward.ones_like(category)*2, category)
        category = awkward.where(SL_WvsQCD_tight_muon_cat, awkward.ones_like(category)*1, category)
