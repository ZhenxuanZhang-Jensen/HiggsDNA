...
class HHWW_Preselection_FHSL(Tagger):
    ...
    def calculate_selection(self, events):
        ...
        fatjet_Hqqqq_cut = fatjet_selections.select_fatjets(
            fatjets = events.FatJet,
            options = self.options["fatjets_Hqqqq"],
            clean = {
                "photons" : {
                    "objects" : events.Diphoton.Photon,
                    "min_dr" : self.options["fatjets_Hqqqq"]["dr_photons"]
                },
                "electrons" : {
                    "objects" : events.SelectedElectron,
                    "min_dr" : self.options["fatjets_Hqqqq"]["dr_electrons"]
                },
                "muons" : {
                    "objects" : events.SelectedMuon,
                    "min_dr" : self.options["fatjets_Hqqqq"]["dr_muons"]
                    },
                },
            name = "SelectedFatJet_Hqqqq",
            tagger = self
        )
        fatjets_Hqqqq = awkward_utils.add_field(
            events = events,
            name = "SelectedFatJet_Hqqqq",
            data = events.FatJet[fatjet_Hqqqq_cut]
        )   

        awkward_utils.add_object_fields(
        events=events,
        name="fatjet_Hqqqq",
        objects=fatjets_Hqqqq[awkward.argsort(fatjets_Hqqqq.Hqqqq_vsQCDTop, ascending=False, axis=-1)],
        n_objects=3,
        dummy_value=-999
        ) 

        fatjet_Hlvqq_cut = fatjet_selections.select_fatjets(
            fatjets = events.FatJet,
            options = self.options["fatjets_Hlvqq"],
            clean = {
                "photons" : {
                    "objects" : events.Diphoton.Photon,
                    "min_dr" : self.options["fatjets_Hlvqq"]["dr_photons"]
                },
                "electrons" : {
                    "objects" : events.SelectedElectron,
                    "min_dr" : self.options["fatjets_Hlvqq"]["dr_electrons"]
                },
                "muons" : {
                    "objects" : events.SelectedMuon,
                    "min_dr" : self.options["fatjets_Hlvqq"]["dr_muons"]
                    },
                },
            name = "SelectedFatJet_Hlvqq",
            tagger = self
        )
        fatjets_Hlvqq = awkward_utils.add_field(
            events = events,
            name = "SelectedFatJet_Hlvqq",
            data = events.FatJet[fatjet_Hlvqq_cut]
        )   

        awkward_utils.add_object_fields(
        events=events,
        name="fatjet_Hlvqq",
        objects=fatjets_Hlvqq[awkward.argsort(fatjets_Hlvqq.Hlvqq_vsQCDTop, ascending=False, axis=-1)],
        n_objects=3,
        dummy_value=-999
        ) 
        ...
        n_fatjets_Hqqqq = awkward.num(fatjets_Hqqqq)
        n_fatjets_Hlvqq = awkward.num(fatjets_Hlvqq)
        awkward_utils.add_field(events,"nGoodAK4jets",n_jets)
        # awkward_utils.add_field(events,"nGood_W_fatjets",n_fatjets_W)
        awkward_utils.add_field(events,"nGood_Hqqqq_fatjets",n_fatjets_Hqqqq)
        awkward_utils.add_field(events,"nGood_Hlvqq_fatjets",n_fatjets_Hlvqq)
        ...
        # If isolated lepton
        SL_FS = (n_leptons == 1) & (n_jets >=2) # fully resovled 2 jets for SL channel with isolated lep                     


        # if no isolated lepton
        # addtional_cut_cat1 = (events.Diphoton.pt > 200) & (events.Diphoton.dR < numpy.pi /2 )
        FH_boosted = (n_leptons == 0) & (n_fatjets_Hqqqq >=1) & (n_fatjets_Hlvqq == 0) 
        SL_boosted = (n_leptons == 0) & (n_fatjets_Hlvqq >= 1) 
        FH_FS = (n_leptons==0) & (n_jets>=4) # 4 jets for FH 
        ...
        category = awkward.where(SL_FS, awkward.ones_like(category)*4, category)
        category = awkward.where(FH_FS, awkward.ones_like(category)*3, category)
        category = awkward.where(FH_boosted, awkward.ones_like(category)*2, category)
        category = awkward.where(SL_boosted, awkward.ones_like(category)*1, category)

        category_cut = (category == 1) | (category == 2)
