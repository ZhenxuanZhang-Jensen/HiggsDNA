...
def select_fatjets(fatjets, subjets, options, clean, name = "none", tagger = None): 
    ...
    # if particleNetMD_Xbb and particleNetMD_QCD are exist, then apply the cut
    if hasattr(fatjets, "particleNetMD_Xbb") and hasattr(fatjets, "particleNetMD_QCD"):
        XbbvsQCD = (fatjets.particleNetMD_Xbb + fatjets.particleNetMD_QCD) / fatjets.particleNetMD_QCD
    else:
        XbbvsQCD = fatjets.pt
    cut_bb_FatJet = XbbvsQCD > options['XbbvsQCD']
    cutTopvsQCD = fatjets.particleNet_TvsQCD > options['TopvsQCD']
    ...
    cut_bb_FatJet=awkward.fill_none(awkward.pad_none(cut_bb_FatJet,1,axis=1),False,axis=-1)
    cutTopvsQCD=awkward.fill_none(awkward.pad_none(cutTopvsQCD,1,axis=1),False,axis=-1)
    ...
    subjets_btag_cut = ((awkward.fill_none(awkward.pad_none(subjets.btagDeepB,1,axis=1),-999,axis=-1)[awkward.fill_none(awkward.pad_none(fatjets.subJetIdx1,1,axis=1),0,axis=-1)]>options['subjet_btagDeepB'])==True) | ((awkward.fill_none(awkward.pad_none(subjets.btagDeepB,1,axis=1),-999,axis=-1)[awkward.fill_none(awkward.pad_none(fatjets.subJetIdx2,1,axis=1),0,axis=-1)]>options['subjet_btagDeepB'])==True)
    
    standard_cuts = awkward.fill_none(awkward.pad_none(standard_cuts,1,axis=1),False,axis=-1)
    all_cuts = standard_cuts & (H_jet_cut) & (cut_bb_FatJet) & (cutTopvsQCD) & (subjets_cut) & (subjets_btag_cut)
    # all_cuts = standard_cuts & (H_jet_cut)

    if tagger is not None:
        tagger.register_cuts(
            names = ["standard_cuts", "H_jet_cut", "cut_bb_FatJet" ,"cutTopvsQCD",  "SubJet_cut", "subjets_btag_cut",  "all_cuts"],
            results = [standard_cuts, H_jet_cut, cut_bb_FatJet, cutTopvsQCD, subjets_cut, subjets_btag_cut, all_cuts],
            cut_type = name
        )
