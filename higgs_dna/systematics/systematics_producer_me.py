...
class SystematicsProducer():
    ...
    def apply_remaining_weight_systs(self, events, syst_tag, tag_idx_map):
        ...
        logger.debug("self.weights.items")
        logger.debug(self.weights.items())
        for weight_name, weight_systs in self.weights.items():
            logger.debug("weight_name")
            logger.debug(weight_name)
            logger.debug("weight_systs")
            logger.debug(weight_systs)
            for weight_syst in weight_systs:
                logger.debug("weight_syst") 
                logger.debug(weight_syst)
                logger.debug("weight_syst.is_applied_all")
                logger.debug(weight_syst.is_applied_all)
                ...
                if hasattr(weight_syst, "modifies_taggers"):
                    ...
                    mask = None
                logger.debug("central_only%s"%central_only)
