...
class WeightSystematic(Systematic):
    ...
    def apply(self, events, syst_tag = NOMINAL_TAG, central_only = False, mask = None, add_fields = False):
        ...
        :rtype: awkward.highlevel.Array
        """
        logger.debug("self.is_produced")
        logger.debug(self.is_produced) #False?
        if not self.is_produced:
            message =  "[WeightSystematic : apply] WeightSystematic: %s must be produced before applying it!" % self.name
            logger.exception(message)
            raise ValueError(message)
        logger.debug("self.is_applied.keys()")
        logger.debug(self.is_applied.keys())
        logger.debug("syst_tag is ")
        logger.debug(syst_tag)
...
class ObjectWeightSystematic(WeightSystematic):
    ...
    def calculate_event_weights(self, events, central_only):
        weights = {}
        logger.debug("calculating event weight")
        logger.debug("branches itemes are ")
        logger.debug(self.branches.items())
        for variation, branch in self.branches.items():
            logger.debug("variation ")
            logger.debug(variation)
            logger.debug("input_collection ")
            logger.debug(self.input_collection)
            logger.debug("target_collection ")
            logger.debug(self.target_collection)
            logger.debug("branch ")
            logger.debug(branch)
            ...
            if self.input_collection == self.target_collection:
                ...
                logger.debug("self.target_collection is tuple")
                if isinstance(branch, tuple):
                    logger.debug("branch is tuple")
                ...
                logger.debug("self.target_collection is not a tuple")
                logger.debug(" branch[1:]")
                logger.debug( branch[1:])
                target_branch = (self.target_collection) + branch[1:]
            logger.debug(events.keys())
            logger.debug(self.name)
            logger.debug(self.normalization_factors)
            logger.debug(target_branch)
            logger.debug(events)
            logger.debug(len(target_branch))
            logger.debug(len(events))
            logger.debug(type(target_branch))
            logger.debug(type(events))
            logger.debug(events["nominal"].fields)
    ...
    # def apply(self, events, syst_tag = f_up, central_only = True, mask = None, add_fields = True):
