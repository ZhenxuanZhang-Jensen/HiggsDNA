...
def run_analysis(config):
    ...
    if events is None: # sometimes this happens when we don't have any data events passing golden json
        ...
        for syst_name, ic_syst in systematics_producer.independent_collections.items():
            logger.debug("syst_name is %s"%syst_name) #fnuf
            logger.debug("ic_syst is %s"%ic_syst)
            
            if not systematics_producer.do_variations:
                continue
            current_time = time.time()
            ics = ic_syst.produce(events)
            logger.debug("ics is %s"%ics)

            t_elapsed_syst += time.time() - current_time        

            for ic_name, events_ic in ics.items():
                logger.debug("ic_name is %s"%ic_name) #up
                logger.debug("events_ic is %s"%events_ic)
                ...
                syst_tag = syst_name + "_" + ic_name #fnuf_up
                logger.debug("syst_tag is %s"%syst_tag)
                current_time = time.time()

                events_ic, tag_idx_map = tag_sequence.run(events_ic, syst_tag)
                logger.debug("events_ic is %s"%events_ic)
                logger.debug("tag_idx_map is %s"%tag_idx_map)
                ...
                #event ic {'nominal': <Array [...] type='13861 * {PuppiNET_ptUp}
                #tag_idx_map {'HHWW_Tagger_combined_FHSL': 0}    
                # systag=fnuf_up
                t_elapsed_syst += time.time() - current_time
