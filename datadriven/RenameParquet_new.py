...
#         print("E1 is negative")
    ...
    mStar = data[name]-data[m1]-data[m2]+250
    data.insert(num+1,'mStar',mStar)
    Dipho_pt_mStar = data["Diphoton_pt"]/data["mStar"]
    FatHbbJet_pt_mStar = data["fathbbjet_pt"]/data["mStar"]
    
    data.insert(num+2,'Dipho_pt_mStar',Dipho_pt_mStar)
    data.insert(num+3,'FatHbbJet_pt_mStar',FatHbbJet_pt_mStar)
#     return mass
...
def load_parquet(fname,isSignal,Xmass,Type):  
    ...
    df["Xmass"]=0
    #df["Type"]=Type
    #df = pd.concat([df,Xmass],axis=1)
    #df = pd.concat([df,Type],axis=1)
    df=df.query("category==1")
    if ("QCD" not in Type):
        GetMinMaxID(df)
    #df["Dipho_pt_mgg"]=df.Diphoton_pt/df.Diphoton_mass
    df_mgg = df.Diphoton_pt/df.Diphoton_mass
    clums = df.shape[1]
    df.insert(clums,'Dipho_pt_mgg',df_mgg)
...
def GetMinMaxID(df):
    ...
    df["minID_genPartFlav"] = df.apply(lambda x: getMinIDFlav(x["Diphoton_lead_pho_mvaID"],x["Diphoton_sublead_pho_mvaID"],x["LeadPhoton_genPartFlav"],x["SubleadPhoton_genPartFlav"]),axis=1)
    # df1 = df.apply(lambda x: getMinID(x["Diphoton_lead_pho_mvaID"],x["Diphoton_sublead_pho_mvaID"]),axis=1)
    ...
    # df2 = df.apply(lambda x: getMaxID(x["Diphoton_lead_pho_mvaID"],x["Diphoton_sublead_pho_mvaID"]),axis=1)
